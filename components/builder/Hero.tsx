    ...
    return <div className="font-inter text-burnt mb-40">
        ...
        <div className="flex mb-14">
            <div className="px-5 py-9 bg-schmarrn-light rounded-[10px]">
                <div  className="font-robotoFlex text-black richtext"
        ...
        <Link href={buttonLink.value !== undefined ? buttonLink.value.data.url : ""} className="px-4 py-1 bg-burnt text-white rounded-full">{buttonText}</Link>
        <p className="text-xs">{alternativeText}</p>
            <ul className="flex gap-4">
                {alternativePlatforms.map((platform: any, i:number) => <li key={i}><a target="_blank" rel="noopener noreferrer" title={platform.title} href={platform.link} ><Image src={platform.logo} width={18} height={18} alt="alt"></Image></a></li>)}
            </ul>
        </div>
    </div>
}