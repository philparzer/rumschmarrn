...
export default function Logo({pathToHome, inverted=false, schmarrntyp}: Props) {
    //TODO: easteregg minecraft font anim
    return <div>
    <Link href={pathToHome} className="flex items-center gap-2 group" title="Home">
    <div className="w-12 h-12">
    {schmarrntyp === undefined ? 
        <svg className="h-full w-full" viewBox="0 0 53 40" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        : schmarrntyp === "Nuss" ? <Nuss />
        : schmarrntyp === "Sahne" ? <Sahne />
        : schmarrntyp === "Apfel" ? <Apfel />
        : schmarrntyp === "Blaubeer" ? <Blaubeer />
        : <>hä?</>
    }
    </div>
